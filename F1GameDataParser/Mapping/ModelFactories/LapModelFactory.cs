...
{
    public class LapModelFactory : ModelFactoryBase<LapPacket, Lap>
    {
        public override Expression<Func<LapPacket, Lap>> ToModelExpression()
        {
            return packet => new Lap
            {
                Header = HeaderExpressionCompiled.Invoke(packet.header),
                LapDetails = packet.lapDetails.Select(lap => new LapDetails
                {
                ...
                TimeTrialPBCarIdx = packet.timeTrialPBCarIdx,
                TimeTrialRivalCarIdx = packet.timeTrialRivalCarIdx
            };
        }
    }
}
